...
void SelectSystem::command(OgreBites::Button* button)
{
	...
	else if (button == OgreFramework::getSingletonPtr()->m_pTrayMgr->getWidget("Extinguish")) //check if extinguish button exits
	{
		...
		if (fireFighter)
		{
			...
			{
				OgreFramework::getSingletonPtr()->m_pLog->logMessage("FireFighter is out of extinguisher foam\n");
			}
	...
	else if (button == OgreFramework::getSingletonPtr()->m_pTrayMgr->getWidget("ReFill"))
	{
		...
		{
			int amountOfFoam = fireFighter->getExtinguisherFoam();
			if (amountOfFoam < fireFighter->getMaxiumumCapacityOfExtinguisherFoam())
			{ //check if the amountOfFoam is less then maximum cappacity
				...
				OgreFramework::getSingletonPtr()->m_pLog->logMessage("Add fire extingshuiser fuel\n");
			}
			else
			{
				OgreFramework::getSingletonPtr()->m_pLog->logMessage("FireFighter is already full with extinguisher foam\n");
			}
		}//if fireFighter has a succesfull cast
		OgreFramework::getSingletonPtr()->m_pLog->logMessage("Refill player extinguisher \n");
	}
}
